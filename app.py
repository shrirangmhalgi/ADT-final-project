...
with open('pickles/data_view/all_data.pkl','rb') as f:
    data = pickle.load(f)
...
def add_college():
    ...
    if request.method == "POST":
        ...
        flag = False #data_view.add_college(college_details)
        if flag:
            flash("College added successfully")
        else:
            flash("Failed to add college. Enter correct values and try again")
        return redirect(url_for('data_view_page'))

@app.route("/edit/<id>", methods=["POST", "GET"])
def get_college_details(id):
    # get id equal to that corresponding id
    college_details = None

    for college in data:
        if college['id'] == int(id):
...
def update_contact(id):
    ...
    if request.method == "POST":
        ...
        flag = False # data_view.update_college_details(college_details)
...
def delete_record(id):
    flag = False# data_view.delete_college_details(id)

    if flag:
        flash("College Deleted Successfully")
    else:
        flash("Failed to delete college.. Please try again later")
    return redirect(url_for('data_view_page'))
